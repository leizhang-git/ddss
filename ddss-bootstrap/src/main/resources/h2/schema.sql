...
CREATE TABLE `ddss_resource`  (
  ...
  INDEX `idx_resource_name`(`resource_name` ASC) 
);
...
CREATE TABLE `ddss_video`  (
  ...
  `cover_image` varchar(500) NULL DEFAULT NULL COMMENT '封面图URL',
  ...
  INDEX `idx_video_name`(`video_name` ASC) 
);
...
CREATE TABLE `gen_table`  (
  ...
  `tpl_web_type` varchar(30) NULL DEFAULT '' COMMENT '前端模板类型（element-ui模版 element-plus模版）',
...
CREATE TABLE `qrtz_triggers`  (
  ...
  `job_data` blob NULL COMMENT '存放持久化job对象',
  PRIMARY KEY (`sched_name`, `trigger_name`, `trigger_group`) ,
  INDEX `sched_name`(`sched_name` ASC, `job_name` ASC, `job_group` ASC) );
...
CREATE TABLE `sys_dict_type`  (
  ...
  `remark` varchar(500) NULL DEFAULT NULL COMMENT '备注',
  PRIMARY KEY (`dict_id`) ,
  UNIQUE INDEX `dict_type`(`dict_type` ASC) 
);
...
CREATE TABLE `sys_logininfor`  (
  ...
  PRIMARY KEY (`info_id`) ,
  INDEX `idx_sys_logininfor_s`(`status` ASC) ,
  INDEX `idx_sys_logininfor_lt`(`login_time` ASC) 
);
...
CREATE TABLE `sys_oper_log`  (
  ...
  PRIMARY KEY (`oper_id`) ,
  INDEX `idx_sys_oper_log_bt`(`business_type` ASC) ,
  INDEX `idx_sys_oper_log_s`(`status` ASC) ,
  INDEX `idx_sys_oper_log_ot`(`oper_time` ASC) 
);
...
CREATE TABLE `sys_video_resource`  (
  ...
  `mime_type` varchar(50) NULL DEFAULT NULL COMMENT '文件类型',
  `description` varchar(500) NULL DEFAULT NULL COMMENT '描述',
  `duration` int NULL DEFAULT NULL COMMENT '时长(秒)',
  `resolution` varchar(20) NULL DEFAULT NULL COMMENT '分辨率',
  `thumbnail` varchar(255) NULL DEFAULT NULL COMMENT '封面图路径',
  `status` char(1) NULL DEFAULT '0' COMMENT '状态（0正常 1停用）',
  `create_by` varchar(64) NULL DEFAULT '' COMMENT '创建者',
  `create_time` datetime NULL DEFAULT NULL COMMENT '创建时间',
  `update_by` varchar(64) NULL DEFAULT '' COMMENT '更新者',
